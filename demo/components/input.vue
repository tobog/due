	...
	<vRow class="demo-layout" flex>
		...
		<vCol lg="10" span="24" class="demo-view">
			...
			<section v-if="show">
				...
				<vSelect class="margin-bottom-20" prefix v-bind="formData" v-model="val1">
	...
	computed: {
		getCode() {
			return `<vInput class="margin-bottom-20" v-bind="${this.getCodeString(this.formData)}" v-model="val">
					...
					<vInput class="margin-bottom-20" type="select" prefix suffix v-bind="${this.getCodeString(this.formData)}" v-model="val1">
						<vInput suffix="search" />
						<vOption value="primary"></vOption>
						<vOption value="ghost"></vOption>
						<vOption value="dashed"></vOption>
		...
		compProps() {
			return [
				...
				{
					prop: 'type',
					explain: "输入框类型，可选值为 button,checkbox,color,date,datetime,datetime-local,month,week,time,email,file,hidden,image,number,password,radio,range,reset,search,submit,tel,text,url",
					type: 'String',
					default: '',
				},
				{
