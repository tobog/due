		...
		<vCol lg="10" span="24" class="demo-view">
			...
			<section v-if="show">
				<vTabs value="name1" v-bind="formData" class="tabsdemo">
					<vTabPane style="height:100px" icon="logo-apple" label="标签标签一标一" name="name1">标签一的内容</vTabPane>
				...
				<vTabs type="card" v-bind="formData" class="tabsdemo">
	...
	computed: {
		...
		compProps() {
			return [
				...
				{
					prop: "TabPane:index",
					explain: "在 TabPane 使用 v-if 时，并不会按照预先的顺序渲染，这时可设置 index，并从小到大排序(需大于 0)",
					type: "Number",
					default: "-"
				},
			];
