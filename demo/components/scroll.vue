        ...
        <vCol lg="10" span="24" class="demo-view">
            ...
            <section v-if="show">
                <vScroll v-bind="formData" :on-reach-edge="handleReachEdge" class="demo-scroll">
                    <div class="padding-15 margin-15" v-for="(item, index) in list" :key="index">
                        Content {{ item }}
                    </div>
                </vScroll>
            </section>
...
export default {
    ...
    computed: {
        getCode() {
            return `<vScroll v-bind="${this.getCodeString(
                this.formData
            )}" :on-reach-edge="handleReachEdge" class="demo-scroll">
