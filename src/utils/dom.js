...
export function scrollTop(el, to = 0, endCallback) {
    ...
    const scroll = function(start, end, step) {
        if (start === end || start <= 0 || start >= el.scrollHeight) {
...
export function scrollIntoView(element, position, behavior = "smooth") {
    if (!element) return
    ...
    if (behavior === "smooth") {
        scrollTop(elsScroll, elsScroll.scrollTop + elePos.top - position)
    } else {
...
export class ClickOut {
    ...
    _handleCallback(event) {
        ...
        // console.log(type, event.timeStamp, this._elements)
        if (type !== "focusout") {
            ...
            this._typeTimeOut = setTimeout(
                ...
                this._currentType === "focusin" ? 300 : 36
            )
            return
        }
