...
export default {
	...
	computed: {
		isInnerColor() {
			return ["confirm", "gray", "success", "primary", "warning", "error", "info", "danger"].indexOf(this.color) > -1;
		},
		showDot() {
		...
		classes() {
			...
			return [
				`${_tobogPrefix_}`,
				{
					...
					[`${_tobogPrefix_}-color-${this.color}`]: !!this.color && this.isInnerColor,
