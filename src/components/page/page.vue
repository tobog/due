...
<template>
	<ul :class="wrapClasses" :data-vue-module="$options.name">
		<li :class="[_tobogPrefix_ + '-total']" v-if="showTotal">
			<slot :total="total">{{langs('page.total','共 '+total+' 条',{total})}}</slot>
		</li>
		<li @click="changePage(currentPage-1)" :title="langs('page.pre','前一页')" v-show="hasTotal">
			<slot name="pre">
		...
		<li @click="changePage(currentPage+1)" :title="langs('page.next','下一页')" v-show="hasTotal">
		...
		<li :class="[_tobogPrefix_+'-select']" v-if="showSizer" v-show="hasTotal">
			<Select v-model="currentPageSize" @on-change="changePageSize" type="select" :transfer="transfer">
				<Option
					...
					:key="index"
					:value="index"
				>{{langs('page.onePageCount',index+'条/每页',{index})}}</Option>
			</Select>
		</li>
