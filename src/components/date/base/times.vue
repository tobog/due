	...
	>
		...
		>
			<li
				...
				v-for="item in hoursCells"
				@click="handleClick(item,'HH')"
			>{{ formatTime(item.date.hours) }}</li>
		</ul>
		<ul
		...
		>
			<li
				:class="getCellCls(item)"
				v-for="item in minutesCells"
				@click="handleClick(item,'MM')"
			>{{ formatTime(item.date.minutes) }}</li>
		</ul>
		<ul
		...
		>
			<li
				:class="getCellCls(item)"
				v-for="item in secondsCells"
				@click="handleClick(item,'SS')"
			>{{ formatTime(item.date.seconds) }}</li>
		</ul>
	</section>
</template>
	...
	methods: {
		...
		handleCell(date, type) {
			return {
				...
				focus:
					compareEqual(date, type, this.foucsDate) &&
					this.foucsDate.key === type
			};
		},
		...
		handleClick(cell, type) {
			...
			if (
				...
				(type === "HH" &&
					!this.hasTimesChild("MM") &&
					!this.hasTimesChild("SS"))
			) {
				type = "times";
			} else {
				type = undefined;
