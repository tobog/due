<template>
	<section :class="classes">
		<div :class="[_tobogPrefix_ + '-bar']">
			<aside v-if="!$slots.extra" :class="[_tobogPrefix_ + '-nav-extra']">
...
<script>
	...
	export default {
		...
		computed: {
			classes() {
				...
				return [
					_tobogPrefix_,
					{
						[`${_tobogPrefix_}-card`]: this.type === "card",
			...
			navMoreClasses() {
				const _tobogPrefix_ = this._tobogPrefix_;
				return [
					`${_tobogPrefix_}-nav-more`,
					{
						[`${_tobogPrefix_}-nav-scroll-disabled`]: this.scrollable
					}
				];
			},
			contentClasses() {
		...
		methods: {
			...
			tabCls(item) {
				...
				return [
					`${_tobogPrefix_}-tab`,
					{
						...
						[`${_tobogPrefix_}-tab-active`]:
							item.name === this.activeKey,
						[`${_tobogPrefix_}-tab-focused`]:
			...
			showClose(item) {
				// console.log(item,this.closable)
				return item.closable === null || item.closable === ""||item.closable === undefined
						? this.closable
						: item.closable;
			},
			scrollPrev() {
				...
				this.navTransform =
					this.navTransform > containerWidth
						? this.navTransform - containerWidth
						: 0;
			},
			scrollNext() {
			...
			updateNavScroll() {
				...
				const containerWidth = this.$refs.navScroll.offsetWidth;
				const currentOffset = this.navTransform;
				console.log(navWidth,containerWidth)
			...
			updateVisibility(index) {
				...
				[...this.$refs.panes.querySelectorAll(childClass)].forEach(
					(el, i) => {
						if (index === i) {
							[...el.children].forEach(
								child =>
									child.classList.contains(childClass) &&
									child.classList.addClass(activeClass)
							);
							...
							setTimeout(() => {
								[...el.children].forEach(
									child =>
										child.classList.contains(childClass) &&
										child.classList.removeClass(activeClass)
								);
							}, transitionTime);
						}
					}
