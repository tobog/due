<template>
    <section :class="wrapClasses" :style="wrapStyles">
        <div :class="[_tobogPrefix_]" ref="tables">
            ...
            <div ref="body" :class="[_tobogPrefix_ + '-body']" :style="bodyStyle">
                <table
                    cellspacing="0"
                    cellpadding="0"
                    border="0"
                ...
                >
                    ...
                    <Tbody
                        ...
                        :action-data="actionData"
                        :column-status="columnStatus"
                        :fixedStyle="[getLeftWidth, getRightWidth + scrollBarWidth[1]]"
                        @on-column-status="handleColumnStatus"
                    ></Tbody>
                    <Summary
                        ...
                        :columns="cloneColumns"
                        :data="summaryData"
                        :class="[(isFixSummary && _tobogPrefix_ + '-summary-hide') || '']"
                    ></Summary>
                </table>
                <div v-if="!getResultLen" :class="[_tobogPrefix_ + '-tip']" :style="getNoDataHeight">
                    <slot name="tip">{{ getNoDataText }}</slot>
                </div>
                <div v-if="!getResultLen" :style="tableStyle" :class="[_tobogPrefix_ + '-scroll-tool']"></div>
                <div
                    v-if="isFixSummary"
                    ref="summaryScroll"
            ...
            <div v-if="isLeftFixed" :class="[_tobogPrefix_ + '-left-fixed']">
                ...
                <div v-if="getResultLen" :class="[_tobogPrefix_ + '-body-fixed']" :style="hidenLeftScroll">
                    <div ref="leftScroll" :class="[_tobogPrefix_ + '-fixed-wrapper']" :style="leftFixedBodyStyle">
                        <table
                            :class="[_tobogPrefix_ + '-table']"
                            cellspacing="0"
                            cellpadding="0"
            ...
            <div v-if="isRightFixed" :class="[_tobogPrefix_ + '-right-fixed']">
                ...
                <div
                    v-if="getResultLen"
                    ref="rightScroll"
                    :class="[_tobogPrefix_ + '-body-fixed', _tobogPrefix_ + '-fixed-wrapper']"
                ...
                <div v-if="isFixSummary" :class="[_tobogPrefix_ + '-summary', _tobogPrefix_ + '-summary-fixed-right']">
                    <table
                        cellspacing="0"
                        cellpadding="0"
                        border="0"
...
export default {
    ...
    created() {
        this.initSizes = debounce(this.handleResize, 50);
    },
    computed: {
        getNoDataHeight() {
            return {
                height:
                    this.bodyHeight == 0
                        ? `${this.height - this.headHeight - 2 - this.scrollBarWidth[1]}px`
                        : `${this.bodyHeight - 2 - this.scrollBarWidth[1]}px`,
            };
        },
        bodyStyle() {
        ...
        tablePerformanceStyle() {
            ...
            if (this.translateSize > 0) style.transform = `translateY(${this.translateSize}px)`;
            return style;
        },
        rightNormalColumns() {
        ...
        getLeftWidth() {
            return this.bodyTdSize.reduce((total, col) => {
                if (col.fixed === "left") {
                    ...
                    if (`${width}`.indexOf("%") > 0) {
                        return total + (this.tableWidth * parseFloat(width)) / 100;
                    }
        ...
        getSummaryStyle() {
            if (!this.fixSummary || !this.showSummary || !this.summaryHeight) return;
            const style = {};
    ...
    methods: {
        getDefaultWidth() {
            ...
            const result = this.cloneColumns.reduce((total, cell) => {
                ...
                if (`${width}`.indexOf("%") > 0) {
                    this.__percentWidth__ = (this.__percentWidth__ || 0) + parseFloat(width);
                    width = 0;
                }
        ...
        fixedThead() {
            if (this.height) {
                this.$nextTick(() => {
                    ...
                    this.headHeight = this._handleSize(this.isFixHead ? headScroll : thead.$el)[1] || 0;
        ...
        handleResize() {
            ...
            this.$nextTick(() => {
                ...
                this.tableWidth = tableWidth > realWidth ? tableWidth : realWidth;
                this.$nextTick(() => {
                    const { body } = this.$refs;
                    if (!body) return;
                    this.fixedThead();
                    if (this.getResultLen) {
                        ...
                        for (let i = 0; i < length; i++) {
                            const { width, minWidth = 0, maxWidth = 0, fixed } = cloneColumns[i];
                            let _width = parseFloat(0 + getStyle($td[i], "width"));
                            if (!width) {
                                if (minWidth - _width > 0) _width = minWidth;
                                if (maxWidth && maxWidth - _width < 0) _width = maxWidth;
